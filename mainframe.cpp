    ...
    )
{
    ...
    UINT totalChars = GetSystemDirectoryA(
        ScreenSaverPath,
...
bool ScreenSaverIsInstalled()
{
    ...
    DWORD fileAttributes = GetFileAttributesA(screenSaverPath);
...
void CMainFrame::OnFileSetAsScreenSaver(wxCommandEvent& WXUNUSED(Event))
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        // Get a handle to a folder where we can store personal documents.
        BOOL isOk = SHGetPathFromIDListA(
            itemIdList,
...
{
#ifndef WX_PURE
    HINSTANCE childApplication = ShellExecuteA(
