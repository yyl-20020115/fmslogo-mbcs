...
#ifndef WX_PURE
LOGFONTA FontRec;                       // record for label font
...
NODE *lsetfocus(NODE *arg)
{
    ...
    else
    {
        ...
        window = FindWindowA(NULL, windowCaption);
    }
...
NODE *lgetfocus(NODE *)
{
    ...
    if (TempH != NULL)
    {
        ::GetWindowTextA(TempH, textbuf, ARRAYSIZE(textbuf));
    }
#endif
...
NODE *lwindowset(NODE *args)
{
    ...
    if (NOT_THROWING)
    {
        ...
        {
            // get handle to Window with arg as Caption
            window = FindWindowA(NULL, caption);
        }
...
int CALLBACK
FindFont(
    const LOGFONTA    *  LogFont,     // pointer to logical-font data
    const TEXTMETRICA *  TextMetric,  // pointer to physical-font data
    ...
    )
{
    ...
    if (!context.found)
    {
        ...
        EnumFontFamiliesA(hdc, NULL, PrintFont, 0L);
    }
...
NODE *lwinhelp(NODE *arg)
{
    ...
    else
    {
        ...
        WinHelpA(GetMainWindow(), textbuf, HELP_INDEX, 0L);
    }
#endif
    return Unbound;
}
...
SIZE labelsize(const char *s)
{
    ...
    HFONT tempFont = CreateFontIndirectA(&FontRec);
    if (tempFont != NULL)
    {
        HFONT oldFont = (HFONT) SelectObject(screen, tempFont);

        GetTextExtentPointA(screen, s, strlen(s), &size);
